...
M.handler = function(_, _, actions)
    ...
    for k, _ in ipairs(lines) do
        if k > 2 then
            vim.api.nvim_buf_set_keymap(popup_bufnr, 'n', string.format('%d', k-2), string.format('<cmd>lua require("config.lspconfig.codeaction").confirm(%d)<CR>', k-2), {})
        end
    end

    vim.api.nvim_command('autocmd CursorMoved <buffer> lua require("seblj.utils.ui").set_cursor()')
    vim.api.nvim_buf_set_keymap(popup_bufnr, 'n', '<CR>', '<cmd>lua require("config.lspconfig.codeaction").confirm()<CR>', {silent = true})
end
